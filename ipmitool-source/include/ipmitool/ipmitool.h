...
#include <ipmitool/ipmi_client.h>
...
ipmi_acquire_ipmb_address(struct ipmi_intf * intf)
{
  ...
  struct ipmi_intf *  bmc_connect(char* username, char *password, char*hostname){
     return ipmi_client(username,password,hostname,ipmitool_cmd_list,NULL);
}

char* get_sdr(struct ipmi_intf *intf, char * name){
    struct sdrData sdr;
    sdr=get_sdr_data(intf, name);
    if (sdr.rc == 0){
        printf("sdr: %s,value:%s*************\n",name,sdr.value);
        return sdr.value;
    }else{
        printf("sdr:%s**,value:  *************\n",name);
        return NULL;
...
void bmc_close(struct ipmi_intf *intf){
    ipmi_cleanup(intf);
...
int run_power_on(struct ipmi_intf *intf){
    int rc =-1;
...
int run_power_off(struct ipmi_intf *intf){
    int rc = -1;
